 ...
 *
 * Beneficios: - Centraliza el manejo de errores: evita try/catch repetidos en
 * controladores. - Respuestas consistentes: devuelve Problem Details (RFC 7807)
 * con estructura uniforme. - Observabilidad: registra logs claros por tipo de
 * error para facilitar el troubleshooting.
 *
 * Qué es: - {@link RestControllerAdvice}: intercepta excepciones lanzadas por
 * controladores REST y transforma los errores en respuestas HTTP
 * estandarizadas. - Usa {@link ProblemDetail} para describir el problema con
 * status, title, detail y propiedades extra. - Usa SLF4J vía Lombok (@Slf4j)
 * para emitir logs con el nivel adecuado.
 */
...
public class GlobalExceptionHandler {
    ...
    public ProblemDetail handleMethodNotSupported(HttpRequestMethodNotSupportedException ex) {
        ...
        problem.setTitle("Método HTTP no soportado");

        String supportedMethods = String.join(", ", ex.getSupportedMethods());
        String detail = String.format("El método '%s' no está permitido para este endpoint. Métodos válidos: %s",
                ex.getMethod(), supportedMethods);
        problem.setDetail(detail);

        problem.setProperty("metodo_usado", ex.getMethod());
        problem.setProperty("metodos_permitidos", ex.getSupportedMethods());
        problem.setType(URI.create("https://http.dev/problems/method-not-allowed"));

        log.warn("Método HTTP no soportado: {} en {}. Métodos permitidos: {}",
                ex.getMethod(), ex.getClass().getSimpleName(), supportedMethods);
